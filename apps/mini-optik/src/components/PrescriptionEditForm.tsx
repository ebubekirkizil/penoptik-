...
export default function PrescriptionEditForm({ rx }: { rx: any }) {
  ...
  const handleDelete = async () => {
    if (!window.confirm("Bu reçeteyi silmek istediğinize emin misiniz? Bu işlem geri alınamaz.")) return;
    
    const toastId = toast.loading("Reçete siliniyor...");
    setDeleting(true);
    setError("");
    ...
    try {
      ...
      toast.success("Reçete başarıyla silindi!", { id: toastId });
      setOpen(false);
      router.refresh();
    } catch (err: any) {
      setError(err.message);
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-4" style={{ backgroundColor: "rgba(0,0,0,0.65)" }}>
      ...
      >
        ...
        {/* Footer */}
        <div className="px-5 py-4 border-t border-[var(--border-color)] flex gap-2 sm:gap-3 shrink-0 flex-wrap">
          <button
            onClick={() => setOpen(false)}
            className="flex-1 py-2.5 text-sm font-semibold text-muted-foreground hover:text-foreground bg-background border border-[var(--border-color)] rounded-xl transition-all min-w-[100px]"
          >
            İptal
          </button>
          <button
            ...
            className="flex-1 py-2.5 text-sm font-semibold text-red-500 hover:text-red-600 bg-red-50 dark:bg-red-500/10 hover:bg-red-100 dark:hover:bg-red-500/20 border border-red-200 dark:border-red-500/20 rounded-xl transition-all flex items-center justify-center gap-2 min-w-[100px]"
          >
          ...
          <button
            ...
            type="submit"
            disabled={saving || deleting}
            className="flex-[2] py-2.5 bg-primary text-white rounded-xl text-sm font-bold flex items-center justify-center gap-2 hover:opacity-90 disabled:opacity-50 transition-all min-w-[150px]"
          >
