...
import {
  ...
  CreditCard, Calendar, AlertCircle, X, ChevronRight, Hash, Printer, Edit
} from "lucide-react";
      ...
      {mounted && selectedOrder && createPortal(
        <div className="fixed inset-0 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm" style={{ zIndex: 99999 }}>
          <div className="bg-surface border border-border-color rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-fade-in-up">
            ...
            <div className="p-4 border-t border-border-color flex justify-between items-center bg-surface/30">
              <div className="flex gap-2">
                <Link
                  href={`/admin/orders/${selectedOrder.id}/print`}
                  className="px-4 py-2 bg-primary text-primary-foreground rounded-xl text-sm font-semibold hover:opacity-90 transition-all flex items-center gap-2 shadow-sm"
                >
                  <Printer className="w-4 h-4" /> <span className="hidden sm:inline">Çıktı Al</span>
                </Link>
                <Link
                  href={`/admin/orders/new?customerId=${customerId}`}
                  className="px-4 py-2 bg-white dark:bg-surface border border-border-color text-foreground rounded-xl text-sm font-semibold hover:bg-surface/5 dark:hover:bg-white/5 transition-all flex items-center gap-2"
                >
                  <Edit className="w-4 h-4" /> <span className="hidden sm:inline">Yeni Sipariş</span>
                </Link>
              </div>
              <button
