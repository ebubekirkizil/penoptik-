...
export default function CustomerTransactionsList({ 
  ...
  const renderBadge = (tx: any) => {
    ...
    const cfg = statusMap[tx.status] || { label: tx.status, bg: "bg-surface", color: "text-muted-foreground", border: "" };
    return (
      <span className={`px-2.5 py-1 rounded-lg text-xs font-semibold inline-flex items-center gap-1 ${cfg.color} ${cfg.bg} ${cfg.border || ""}`}>
        {cfg.label}
      </span>
    );
  };
