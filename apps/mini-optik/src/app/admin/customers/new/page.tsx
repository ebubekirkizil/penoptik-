...
export default function NewCustomerWithScannerPage() {
  ...
  useEffect(() => {
    const pendingData = sessionStorage.getItem("pendingAiCustomer");
    if (pendingData) {
      try {
        const parsed = JSON.parse(pendingData);
        setForm(prev => ({ ...prev, ...parsed }));
        setShowPrescription(true);
        toast.success("AI reçeteyi okudu. Bilgileri kontrol edip kaydedin.");
      } catch (err) {
        console.error("Failed to parse pending ai customer", err);
      } finally {
        sessionStorage.removeItem("pendingAiCustomer");
      }
    }
  }, []);
  ...
  const handleParse = async (fileToParse?: File) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || "Okuma başarısız");

      setForm(prev => ({ ...prev, ...data }));
      toast.success("Reçete başarıyla okundu ve form dolduruldu!");
    } catch (err: any) {
      setError(err.message ?? "Bir hata oluştu.");
      toast.error("Reçete okuma hatası: " + (err.message ?? "Bilinmeyen Hata"));
    } finally {
      setParsing(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.firstName || !form.lastName) {
      toast.error("Müşteri adı ve soyadı zorunludur.");
      return;
    }

    let cleaned = form.phone ? form.phone.replace(/\D/g, "") : "";
    if (!cleaned || cleaned.length < 10) {
      const proceed = window.confirm("Telefon numarası girmediniz veya eksik girdiniz. Müşteri telefon numarası olmadan sisteme kaydedilsin mi?");
      if (!proceed) return;
      cleaned = ""; // Backend'in otomatik numara ataması için boş gönder
    }
    ...
    try {
      ...
      if (hasEyeInfo) {
        ...
        if (!eyeRes.ok) {
          ...
          throw new Error("Müşteri kaydedildi fakat göz bilgileri kaydedilemedi: " + (eyeData.error || "Bilinmeyen hata"));
        }
      }

      // 3. Create Order if present in AI draft
      if (form.productName || form.totalPrice) {
        const orderRes = await fetch("/api/orders", {
          ...
          body: JSON.stringify({
            ...
            totalPrice: parseFloat(String(form.totalPrice)) || 0,
            deposit: parseFloat(String(form.downPayment)) || 0,
            balance: Math.max(0, (parseFloat(String(form.totalPrice)) || 0) - (parseFloat(String(form.downPayment)) || 0)),
            status: form.deliveryStatus || "PENDING",
            deliveryDate: form.deliveryDate || null,
            installmentCount: parseInt(String(form.installmentCount)) || 1,
            manualInstallments: form.installments?.map(i => ({ amount: i.amount, dueDate: i.date })) || [],
            installmentMode: form.installments && form.installments.length > 0 ? "MANUAL" : "AUTO"
          }),
        });
        if (!orderRes.ok) {
           const orderData = await orderRes.json();
           throw new Error("Müşteri kaydedildi fakat sipariş kaydedilemedi: " + (orderData.error || "Bilinmeyen hata"));
        }
      }

      toast.success("Müşteri ve Sipariş başarıyla kaydedildi!", { id: toastId });
      
      // Navigate to the newly created customer page
      if (cusData.generatedPassword) {
        router.push(`/admin/customers/${cusData.id}?newPw=${cusData.generatedPassword}`);
      } else {
        router.push(`/admin/customers/${cusData.id}`);
      }
    } catch (err: any) {
      setError(err.message ?? "Bir hata oluştu.");
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-foreground">Yeni Müşteri & Göz Bilgileri</h1>
          </div>
        </div>
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-[#5c9ca8] to-[#3a7c88] text-white hover:opacity-90 rounded-xl font-bold transition-all shadow-md shrink-0"
        >
          <Camera className="w-4 h-4" />
          Kamera ile Hızlı Müşteri Oluştur
        </button>
      </div>
      ...
      {parsing && (
        <div className="fixed inset-0 z-[9999] bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center text-white">
          <div className="w-16 h-16 border-4 border-[#5c9ca8] border-t-transparent rounded-full animate-spin mb-4" />
          <h2 className="text-xl font-bold">Yapay Zeka Reçeteyi Okuyor...</h2>
          <p className="text-white/70 mt-2">Bu işlem birkaç saniye sürebilir, lütfen bekleyin.</p>
        </div>
      )}
      {error && (
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white dark:bg-surface shadow-lg shadow-slate-200/50 dark:shadow-none rounded-3xl p-6 md:p-8 space-y-6 border border-slate-100 dark:border-border-color/50 dark:border-none">
            <h3 className="text-foreground font-semibold text-lg  pb-3 mb-4">Müşteri Bilgileri</h3>
        ...
        <div className="flex gap-4">
          <Link href="/admin/customers" className="flex-1 glass  text-muted-foreground py-4 rounded-xl font-bold text-center hover:text-foreground transition-all">
            İptal
          </Link>
          <button type="submit" disabled={loading} className="flex-[2] gradient-primary text-[#1B242A] py-4 rounded-xl font-bold hover:opacity-90 hover:scale-[1.02] disabled:opacity-60 disabled:scale-100 transition-all glow-primary flex items-center justify-center gap-2 text-lg">
            {loading ? "Kaydediliyor..." : <><Save className="w-5 h-5" /> Müşteriyi Kaydet</>}
          </button>
        </div>
      </form>
    </div>
