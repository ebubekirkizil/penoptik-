...
async function getCustomers(q: string, filter: string) {
...
async function CustomersData({ q, filter }: { q: string, filter: string }) {
  ...
  const customers = await getCustomers(q, filter);
    ...
    <div className="">
      ...
      {customers.length === 0 && (
        <div className="card p-16 text-center flex flex-col items-center gap-4">
          ...
          <div>
            <p className="font-black text-foreground text-xl">Henüz Müşteri Yok</p>
            <p className="text-muted-foreground text-sm mt-1">Yeni müşteri ekleyerek başlayın.</p>
          </div>
          <Link href="/admin/customers/new" className="btn-primary mt-2">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-background border-b border-[var(--border-color)]">
                  <tr>
                    <th className="px-5 py-3.5 font-semibold text-muted-foreground text-xs uppercase tracking-wider">Müşteri</th>
                    <th className="px-5 py-3.5 font-semibold text-muted-foreground text-xs uppercase tracking-wider">İletişim</th>
                    <th className="px-5 py-3.5 font-semibold text-muted-foreground text-xs uppercase tracking-wider">Son Sipariş</th>
                    <th className="px-5 py-3.5 font-semibold text-muted-foreground text-xs uppercase tracking-wider">Kayıt</th>
                    <th className="px-5 py-3.5 font-semibold text-muted-foreground text-xs uppercase tracking-wider text-right">İşlem</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[var(--border-color)]">
                  {customers.map((customer) => {
                    const lastOrder = customer.opticOrders[0];
                    const cfg = lastOrder ? statusMap[lastOrder.status] || { label: lastOrder.status, bg: "bg-surface", color: "text-muted-foreground", border: "" } : null;
                    return (
                      <ClickableRow
                        key={customer.id}
                      ...
                      >
                        ...
                        <td className="px-5 py-4">
                          {cfg && lastOrder ? (
                            <div className="space-y-1">
                              <span className={`inline-flex px-2.5 py-0.5 rounded-full text-[11px] font-semibold border ${cfg.bg} ${cfg.color} ${cfg.border || ""}`}>
                                {cfg.label}
                              </span>
                            ...
                            <span className="text-xs text-muted-foreground italic">Sipariş yok</span>
                          )}
                        </td>
                        <td className="px-5 py-4">
                        ...
                        <td className="px-5 py-4 text-right">
                          <span
                            className="inline-flex items-center gap-1 text-xs font-semibold text-primary group-hover:text-primary-dark transition-colors"
                          >
            ...
            {customers.map((customer) => {
              ...
              return (
                <ClickableCard
                  key={customer.id}
                  href={`/admin/customers/${customer.id}`}
                  className="card flex items-center gap-4 p-4 transition-transform"
                >
                  {/* Avatar */}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      <span className="text-[11px] text-muted-foreground bg-background px-1.5 py-0.5 rounded-md border border-[var(--border-color)]">
                        {customer._count.opticOrders} sipariş
                      </span>
                      {cfg && lastOrder && (
  ...
  return (
    <div className="page-container space-y-5">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-foreground tracking-tight">Müşteriler</h1>
          <p className="text-muted-foreground text-sm mt-1">
            Kayıtlı müşterileriniz
          </p>
        </div>
