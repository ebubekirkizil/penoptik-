...
export default async function EcommerceOrdersPage() {
  ...
  if (!activeFeatures.includes("MOD_ECOMMERCE_ORDERS")) {
    return (
      <div className="p-6 md:p-10 max-w-4xl mx-auto text-center mt-20">
        ...
        <p className="text-slate-500 mb-8">Bu firmaya ait E-Ticaret ve Sipariş Yönetimi (OMS) modülü aktif değildir. Özelliği kullanmak için Enterprise pakete geçiş yapmalısınız.</p>
  ...
  // Eğer yetkisi varsa, siparişleri getir
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-2 text-sm">
            B2B ve B2C siparişlerinizi, kargo takiplerini ve iade (RMA) süreçlerini buradan yönetin.
          </p>
        </div>
      </div>
              ...
              {orders.length === 0 ? (
                ...
                orders.map((order) => (
                  <tr key={order.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      {order.status === "PENDING" && <span className="text-amber-600 bg-amber-50 dark:bg-amber-500/10 px-2.5 py-1 rounded-lg text-xs font-medium">Yeni Sipariş</span>}
