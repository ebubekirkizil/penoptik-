...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!identifier || !password) {
      return NextResponse.json({ error: "Giriş bilgileri ve şifre zorunludur." }, { status: 400 });
    }
    ...
    if (adminUser && adminUser.password) {
      ...
      if (isMatch) {
        ...
        response.cookies.set("userId", adminUser.id, { httpOnly: false, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/", maxAge: 2592000 });
        if (adminUser.firmId) {
          response.cookies.set("firmId", adminUser.firmId, { httpOnly: false, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/", maxAge: 2592000 });
        }
        return response;
      }
    }
    ...
    if (!customer.password) {
      return NextResponse.json({ error: "Bu hesaba ait şifre bulunmuyor. Lütfen optisyeninizle iletişime geçin." }, { status: 401 });
    }
    ...
    if (customer.isPasswordTemporary) {
      if (customer.tempPasswordExpires && new Date(customer.tempPasswordExpires) < new Date()) {
        return NextResponse.json({ error: "Geçici şifrenizin süresi (7 gün) dolmuş. Lütfen optisyeninizle iletişime geçip yeni şifre talep edin." }, { status: 403 });
      }
      return NextResponse.json({
        success: true,
    ...
    console.error(error);
    return NextResponse.json({ error: "Giriş başarısız. Lütfen bilgilerinizi kontrol edin." }, { status: 500 });
  }
}
