...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const customerName = order?.customer ? `${order.customer.firstName} ${order.customer.lastName}` : "Bilinmeyen Müşteri";
    ...
    if (autoCount && autoCount > 1 && balance) {
      ...
      await logAction(`Otomatik Taksit Planı Oluşturuldu: ${customerName}`, {
        orderId,
        count,
