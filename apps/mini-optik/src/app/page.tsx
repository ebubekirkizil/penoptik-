...
import { ArrowRight, Eye, ShieldCheck, Clock, Search, ShoppingBag, Phone, MapPin, ChevronRight, CheckCircle2 } from "lucide-react";
import { ThemeToggle } from "@/components/ThemeToggle";

export default function PenOptikLanding() {
  const [mounted, setMounted] = useState(false);
  const [firm, setFirm] = useState<{ phone?: string, address?: string, logoUrl?: string, name?: string, useLogo?: boolean, mapUrl?: string } | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/settings")
      .then(res => res.json())
      .then(data => {
        const logoUsage = data?.themeData?.logoUsage || { system: true, print: true };
        if (data.firm) {
          setFirm({ ...data.firm, useLogo: logoUsage.system, mapUrl: data.themeData?.mapUrl });
        }
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <main className="min-h-screen bg-background relative overflow-hidden flex flex-col selection:bg-primary/20 selection:text-primary">
      ...
      <header className="relative z-50 w-full border-b border-border/50 bg-background/70 backdrop-blur-xl supports-[backdrop-filter]:bg-background/40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-20 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3 group">
            {firm?.logoUrl && firm?.useLogo !== false ? (
              <img src={firm.logoUrl} alt={firm.name || "Logo"} className="h-10 sm:h-12 object-contain" />
            ) : (
              <div className="relative w-12 h-12 flex items-center justify-center rounded-2xl bg-gradient-to-br from-primary to-primary/80 text-white shadow-lg shadow-primary/20 transition-transform group-hover:scale-105">
                <span className="font-black text-2xl tracking-tighter">{firm?.name ? firm.name.charAt(0) : "P"}</span>
                <div className="absolute inset-0 rounded-2xl ring-1 ring-white/20"></div>
              </div>
            )}
            <div className="flex flex-col">
              {firm?.logoUrl && firm?.useLogo !== false ? null : (
                <span className="font-black text-xl sm:text-xl tracking-tight text-foreground leading-none">{firm?.name || "PEN OPTİK"}</span>
              )}
        ...
        <div className="max-w-7xl mx-auto w-full grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <p className="text-lg sm:text-xl text-muted-foreground max-w-lg mb-10 font-medium leading-relaxed">
              Sistemimiz ile reçetelerinizi dijitalde saklayın, gözlük ölçümlerinizi takip edin ve sipariş durumunuzu saniyeler içinde sorgulayın.
            </p>
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-bold text-sm text-foreground">Sipariş #PN-9823</div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-xs font-semibold text-muted-foreground">
                  <span>Montaj</span>
                  <span className="text-blue-500">Teslimat Yaklaştı</span>
                </div>
              </div>
            </motion.div>

            {/* Card 2: Digital Prescription */}
            <motion.div 
              initial={{ y: 0 }}
              animate={{ y: 15 }}
      ...
      <section id="hizmetler" className="relative z-20 py-24 bg-surface/30 border-y border-border/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center max-w-2xl mx-auto mb-16">
            ...
            <p className="text-muted-foreground font-medium">Geleneksel optik anlayışını dijitalin gücüyle harmanlıyor, tüm süreçlerinizi şeffaf bir şekilde yönetiyoruz.</p>
          </div>
            ...
            {[
              {
                icon: Clock,
                title: "Anlık Takip",
                desc: "Siparişinizin cam kesiminden montaja kadar tüm aşamalarını SMS beklemeden online görüntüleyin.",
                color: "text-blue-500",
                bg: "bg-blue-500/10",
              ...
              },
              {
                icon: ShieldCheck,
                title: "Reçete Arşivi",
                desc: "Yıllar önceki reçetelerinize ve ölçümlerinize sistemimizden saniyeler içinde güvenle ulaşın.",
              ...
              },
              {
                icon: ShoppingBag,
                title: "Bakiye & Ödeme",
                desc: "Geçmiş ödemelerinizi, taksitlerinizi ve hesap ekstrenizi şeffaf ve düzenli bir şekilde kontrol edin.",
      ...
      {/* Footer */}
      <footer className="relative z-20 bg-background pt-16 pb-8 border-t border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="grid md:grid-cols-4 gap-12 mb-12">
            <div className="md:col-span-2">
              <div className="flex items-center gap-2 mb-6">
                <div className="w-8 h-8 rounded-lg bg-primary text-white flex items-center justify-center font-black text-lg">P</div>
                <span className="font-black text-xl tracking-tight">{firm?.name || "PEN OPTİK"}</span>
              </div>
              <p className="text-muted-foreground font-medium max-w-sm mb-6">
                Göz sağlığınız ve stiliniz için en iyi çözümleri sunuyoruz. Teknolojiyi kullanarak hayatınızı kolaylaştırıyoruz.
              </p>
            ...
            <div>
              ...
              <ul className="space-y-4 text-sm font-medium text-muted-foreground">
                <li><Link href="/track" className="hover:text-primary transition-colors">Sipariş Sorgula</Link></li>
                <li><Link href="/login" className="hover:text-primary transition-colors">Müşteri Girişi</Link></li>
                <li><Link href="#hizmetler" className="hover:text-primary transition-colors">Hizmetlerimiz</Link></li>
              </ul>
            </div>

            <div>
              <h4 className="font-bold text-foreground mb-6">İletişim</h4>
              <ul className="space-y-4 text-sm font-medium text-muted-foreground">
                <li>
                  <a href={`tel:${(firm?.phone || "02163900444").replace(/[^0-9+]/g, '')}`} className="flex items-center gap-3 hover:text-primary transition-colors group">
                    <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center group-hover:bg-primary/20 transition-colors">
                      <Phone className="w-4 h-4 text-primary" />
                    </div>
                    <span>{firm?.phone || "0216 390 04 44"}</span>
                  </a>
                </li>
                <li>
                  <a 
                    href={firm?.mapUrl || (firm?.address && firm.address.startsWith("http") ? firm.address : "https://maps.google.com/?q=Batı+Mah.,+İsmetpaşa+Cad.,+No:+33/35A,+Pendik+/+İstanbul")} 
                    target="_blank" 
                  ...
                  >
                    ...
                    <span className="leading-relaxed text-left">{firm?.address && !firm.address.startsWith("http") ? firm.address : "Batı Mah., İsmetpaşa Cad., No: 33/35A, Pendik / İstanbul"}</span>
                  </a>
                </li>
              </ul>
            </div>
