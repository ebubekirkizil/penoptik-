...
import { Glasses, Camera, ChevronRight, Plus } from "lucide-react";
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-8 gap-4">
        ...
        <div className="flex items-center gap-3 w-full lg:w-auto">
          ...
          <Link
            ...
            className="btn-primary inline-flex items-center gap-2 flex-shrink-0 whitespace-nowrap"
          >
            <Plus className="w-4 h-4" />
            <span className="hidden sm:inline">Yeni Göz Bilgisi</span>
      ...
      {prescriptions.length === 0 ? (
        <div className="bg-white dark:bg-surface shadow-sm rounded-2xl p-16 text-center  flex flex-col items-center">
          ...
          <p className="text-muted-foreground text-sm mb-6">Müxteri detayından yeni göz bilgisi (reçete) ekleyebilirsiniz.</p>
          <Link href="/demo/sample-optic/customers" className="btn-primary px-6 py-3 rounded-xl font-medium flex items-center gap-2">
            <Plus className="w-4 h-4" /> Müxterilere Git
          </Link>
        </div>
      ) : (
        <div className="space-y-3">
          {prescriptions.map((rx) => (
            <Link
              key={rx.id}
              href={`/demo/sample-optic/customers/${rx.customerId}`}
