    ...
    <div className="p-6 md:p-10 max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          <Link href="/demo/sample-optic/ecommerce/orders" className="p-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 rounded-xl transition-colors">
            <ArrowLeft className="w-5 h-5 text-slate-600 dark:text-slate-300" />
          </Link>
          <div>
            <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-3">
              Siparix #{order.orderNumber}
            </h1>
            <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm flex items-center gap-2">
              {new Date(order.createdAt).toLocaleString("tr-TR")}
              <span className="w-1 h-1 rounded-full bg-slate-300 dark:bg-slate-700"></span>
              {order.orderType === "B2B" ? "Toptan / Bayi Siparixi" : "B2C Perakende Siparixi"}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-6 shadow-sm">
            <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
              <Package className="w-5 h-5 text-indigo-500" /> Siparix İçeriği
            </h2>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-6 shadow-sm">
            ...
            {order.paymentStatus === "PAID" && (
              <div className="mt-6 p-4 bg-emerald-50 dark:bg-emerald-500/10 rounded-xl flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-emerald-700 dark:text-emerald-400">Ödeme Baxarılı</p>
        ...
        {/* Sağ Kolon: Müxteri, Adres ve Durum */}
        <div className="space-y-8">
          
          {/* Müxteri Bilgileri */}
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-6 shadow-sm">
            <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
              <MapPin className="w-5 h-5 text-sky-500" /> Müxteri & Teslimat
            </h2>
            <div className="space-y-4 text-sm">
              <div>
                <p className="text-slate-500 mb-1">Müxteri</p>
          ...
          {/* Audit Log / Siparix Geçmixi */}
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-6 shadow-sm">
            <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
              <FileText className="w-5 h-5 text-amber-500" /> İxlem Geçmixi
            </h2>
