...
export default function TrashPage() {
  ...
  const handleAction = async (id: string, type: string, action: "RESTORE" | "HARD_DELETE") => {
    if (action === "HARD_DELETE" && !confirm("Bu öğeyi kalıcı olarak silmek istediğinize emin misiniz? Bu ixlem geri alınamaz.")) {
      return;
    }

    const toastId = toast.loading("İxlem yapılıyor...");
    try {
      const res = await fetch("/api/trash", {
        method: "POST",
      ...
      if (res.ok) {
        toast.success(action === "RESTORE" ? "Öğe baxarıyla geri yüklendi!" : "Öğe kalıcı olarak silindi.", { id: toastId });
        fetchItems();
      } else {
        toast.error(data.error || "İxlem baxarısız.", { id: toastId });
      }
    } catch (e: any) {
      toast.error("Bir hata oluxtu.", { id: toastId });
    }
  };
  ...
  const getTypeLabel = (type: string) => {
    if (type === 'CUSTOMER') return "Müxteri";
    if (type === 'ORDER') return "Siparix";
  ...
  const getDetailText = (item: any) => {
    ...
    if (item.type === 'ORDER') return `Siparix No: ${item.id.slice(-6)}`;
    ...
    <div className="page-container space-y-5">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Silinen müxteriler, siparixler ve reçeteler 7 gün boyunca burada saklanır.
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="card p-16 text-center flex flex-col items-center gap-4 animate-in fade-in">
          ...
          <div>
            <p className="font-black text-foreground text-xl">Çöp Kutusu Box</p>
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-background border-b border-[var(--border-color)]">
                <tr>
                  ...
                  <th className="px-5 py-3.5 font-semibold text-muted-foreground text-xs uppercase tracking-wider text-right">İxlemler</th>
                </tr>
              </thead>
