...
import {
  ...
  AlertCircle, TrendingUp, BarChart3, Hash, X
} from "lucide-react";

const STATUS_CONFIG = {
  PENDING:   { label: "Bekliyor",      icon: <ClipboardList className="w-4 h-4 drop-shadow-sm" />, color: "text-amber-700 dark:text-amber-200",  bg: "bg-gradient-to-br from-amber-500/20 to-orange-500/10 border-amber-500/30 shadow-[inset_0_1px_0_rgba(255,255,255,0.2)] dark:shadow-none", step: 1 },
  PREPARING: { label: "Hazırlanıyor",  icon: <Clock className="w-4 h-4 drop-shadow-sm" />,         color: "text-blue-700 dark:text-blue-200",    bg: "bg-gradient-to-br from-blue-500/20 to-indigo-500/10 border-blue-500/30 shadow-[inset_0_1px_0_rgba(255,255,255,0.2)] dark:shadow-none",     step: 2 },
  READY:     { label: "Teslime Hazır", icon: <CheckCircle className="w-4 h-4 drop-shadow-sm" />,   color: "text-emerald-700 dark:text-emerald-200", bg: "bg-gradient-to-br from-emerald-500/20 to-teal-500/10 border-emerald-500/30 shadow-[inset_0_1px_0_rgba(255,255,255,0.2)] dark:shadow-none", step: 3 },
  DELIVERED: { label: "Teslim Edildi", icon: <Check className="w-4 h-4" />,         color: "text-muted-foreground", bg: "bg-surface/50 border-border-color", step: 4 },
  COMPLETED: { label: "Tamamlandı", icon: <CheckCircle className="w-4 h-4" />, color: "text-primary", bg: "bg-primary/10 border-primary/20", step: 5 },
};
...
export default async function CustomerDetailPage({
  ...
  // Fetch ALL logs for this customer to map users to transactions
  ...
  customer.opticOrders.forEach(order => {
    ...
    transactions.push({
      ...
      title: order.products || "Siparix (Belirtilmemix)",
    ...
    order.installments?.forEach(inst => {
      if (inst.isPaid) {
        ...
        transactions.push({
          ...
          title: `Taksit Ödemesi (${order.products || "Siparix"})`,
          amount: inst.amount,
          status: "PAID",
          type: "INSTALLMENT",
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
      ...
      <div className="bg-white dark:bg-surface shadow-sm rounded-2xl p-6 mb-5 ">
        <div className="flex items-start gap-4">
          ...
          <div className="flex flex-col gap-2 flex-shrink-0">
            <Link
              href={`/demo/sample-optic/customers/${id}/edit`}
            ...
            <Link
              href={`/demo/sample-optic/orders/new?customerId=${id}`}
      ...
      <div className="bg-white dark:bg-surface shadow-sm rounded-2xl  mb-5">
        ...
        {customer.opticOrders.length === 0 ? (
          <div className="p-8 text-center text-muted-foreground flex flex-col items-center">
            ...
            <Link href={`/demo/sample-optic/orders/new?customerId=${id}`} className="mt-3 text-primary text-sm font-medium hover:underline">
              + Siparix Ekle
            </Link>
          </div>
        ) : (
      ...
      {/* ─── Son İxlemler ─── */}
      <CustomerTransactionsList transactions={transactions} />
    </div>
  );
}
