...
export default function EditCustomerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    params.then(({ id: resolvedId }) => {
      ...
      fetch(`/api/customers/${resolvedId}`)
        ...
        .catch(() => setError("Müxteri bilgileri yüklenemedi."))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || "Güncelleme baxarısız.");

      setSaved(true);
      setTimeout(() => {
        router.refresh();
        router.push(`/demo/sample-optic/customers/${id}`);
      }, 1000);
    } catch (err: any) {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        <Link href={`/demo/sample-optic/customers/${id}`} className="text-muted-foreground hover:text-foreground transition-colors text-sm flex items-center gap-1">
          <ArrowLeft className="w-4 h-4" /> Müxteri Detayı
        </Link>
      </div>
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-black text-foreground">Müxteri Düzenle</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white dark:bg-surface shadow-sm rounded-2xl p-6  space-y-4">
          <h3 className="text-foreground font-semibold flex items-center gap-2  pb-3">
            <User className="w-4 h-4 text-primary" /> Kixisel Bilgiler
          </h3>
        ...
        <div className="flex gap-3">
          <Link
            href={`/demo/sample-optic/customers/${id}`}
          ...
          >
            {saving ? (
              ...
              <><Save className="w-5 h-5" /> Değixiklikleri Kaydet</>
            )}
          </button>
        </div>
      </form>
