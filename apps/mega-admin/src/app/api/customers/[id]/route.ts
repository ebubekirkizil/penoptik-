...
export async function PUT(
  ...
  try {
    ...
    if (!existingCust) return NextResponse.json({ error: "Müxteri bulunamadı." }, { status: 404 });
    ...
    if (conflict) {
      return NextResponse.json({ error: "Bu telefon numarası baxka bir müxteriye ait." }, { status: 409 });
    }
    ...
    if (body.isCustomerRequest) {
      ...
      return NextResponse.json({ message: "Doğrulama isteği oluxturuldu" });
    }
    ...
    await logAction(`Müxteri Bilgileri Güncellendi: ${updated.firstName} ${updated.lastName}`, {
      old: oldDataForLog,
      new: dataToUpdate
    });
...
export async function DELETE(
  ...
  try {
    ...
    if (existing) {
      await logAction(`Müxteri Silindi: ${existing.firstName} ${existing.lastName}`, {
