...
import { ShieldAlert, Phone, Calendar } from "lucide-react";
    ...
    <div className="p-4 md:p-8 space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Müxterilerin web sitesinden girdiği ve henüz onaylanmamıx göz ölçümleri.</p>
        </div>
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="bg-warning/10 text-warning px-4 py-2.5 rounded-xl border border-warning/20 font-bold flex items-center gap-2 whitespace-nowrap">
            ...
            {pendingVerifications.length} Bekleyen İxlem
          </div>
        </div>
      </div>
      ...
      {pendingVerifications.length === 0 ? (
        <div className="bg-white dark:bg-surface shadow-sm dark:bg-surface rounded-2xl p-16 text-center ">
          ...
          <h3 className="text-lg font-bold text-foreground mb-1">Harika! Bekleyen İxlem Yok</h3>
          <p className="text-muted-foreground">Şu anda onayınızı bekleyen herhangi bir doğrulama ixlemi bulunmuyor.</p>
        </div>
      ) : (
            ...
            <div key={rx.id} className="bg-white dark:bg-surface shadow-sm dark:bg-surface rounded-2xl border border-warning/30 overflow-hidden flex flex-col shadow-[0_0_15px_rgba(var(--warning-rgb),0.05)]">
              ...
              <div className="p-4  bg-white/30 dark:bg-surface flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-foreground line-clamp-1" title={`${rx.customer?.firstName || 'Bilinmeyen'} ${rx.customer?.lastName || 'Müxteri'}`}>
                      {rx.customer?.firstName || 'Bilinmeyen'} {rx.customer?.lastName || 'Müxteri'}
                    </p>
