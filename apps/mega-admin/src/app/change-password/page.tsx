...
export default function ChangePasswordPage() {
  ...
  useEffect(() => {
    const id = sessionStorage.getItem("tempCustomerId");
    const phone = sessionStorage.getItem("tempCustomerPhone");
    const temp = sessionStorage.getItem("tempCustomerPassword");

    if (!id || !temp) {
      router.push("/login"); // Güvenlik: Eğer session storage boxsa login'e geri at.
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (newPassword.length < 6) {
      setError("Yeni xifreniz en az 6 karakter olmalıdır.");
      return;
    }

    if (newPassword !== confirmPassword) {
      setError("Şifreler uyuxmuyor.");
      return;
    }

    if (newPassword === tempPassword) {
      setError("Yeni xifreniz geçici xifrenizle aynı olamaz.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || "Şifre değixtirilemedi.");
      ...
      // Baxarılı girix: Takip sayfasına at
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground text-sm">
            Güvenliğiniz için lütfen geçici xifrenizi kalıcı ve güvenli bir xifreyle değixtirin.
          </p>
        </div>
        ...
        <div className="glass rounded-3xl p-8 shadow-2xl border border-border-color">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            >
              ...
              {loading ? "Kaydediliyor..." : "Şifremi Kaydet ve Girix Yap"}
            </button>
          </form>
        </div>
      </div>
