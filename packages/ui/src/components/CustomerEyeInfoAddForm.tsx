...
import { Plus, X, Save, Loader2, Eye } from "lucide-react";

interface CustomerEyeInfoAddFormProps {
  customerId: string;
}

export default function CustomerEyeInfoAddForm({ customerId }: CustomerEyeInfoAddFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/prescriptions", {
        ...
        body: JSON.stringify({ ...form, customerId, isPending: true }),
      });

      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.error || "Göz bilgisi kaydedilirken bir hata oluştu.");
      }
  ...
  if (!open) {
    return (
      <button
        ...
        className="glass border border-border-color text-primary hover:text-foreground hover:border-primary/40 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all flex items-center gap-1.5 cursor-pointer ml-auto"
      >
  ...
  const modalContent = (
    <div className="fixed inset-0 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm overflow-hidden" style={{ zIndex: 99999 }}>
      <div className="bg-surface border border-border-color rounded-2xl w-full max-w-4xl shadow-2xl flex flex-col max-h-[95vh]">
        <div className="p-4 border-b border-border-color flex items-center justify-between">
          <h2 className="text-foreground font-bold flex items-center gap-2">
            <Eye className="w-5 h-5 text-primary" /> Yeni Göz Ölçümü Ekle
          </h2>
        ...
        <div className="p-4 overflow-y-auto">
          <form id="eye-info-form" onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="mt-4 pt-2 border-t border-border-color/50">
              <label className="block text-[10px] uppercase font-semibold text-muted-foreground mb-2">Açıklama / Notlar</label>
              <textarea
                rows={2}
                value={form.notes || ""}
        ...
        <div className="p-4 border-t border-border-color flex gap-3">
          <button onClick={() => setOpen(false)} className="flex-1 py-2 text-sm font-semibold text-muted-foreground hover:text-foreground cursor-pointer">İptal</button>
          <button form="eye-info-form" type="submit" disabled={saving} className="flex-[2] py-2 gradient-primary rounded-lg text-[#1B242A] text-sm font-bold flex items-center justify-center gap-2 hover:opacity-90 disabled:opacity-50 cursor-pointer">
