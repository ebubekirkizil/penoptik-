...
export default function FinancialChart({ orders }: { orders: Order[] }) {
  ...
  const data = useMemo(() => {
    ...
    let result = Object.values(grouped).sort((a, b) => a.sortKey - b.sortKey);
    ...
    if (result.length === 0) {
      ...
      for (let i = daysToGenerate - 1; i >= 0; i--) {
        ...
        const dailyIncome = Math.round(baseIncome * randomFactor);
        const dailyDeposit = Math.round(baseDeposit * randomFactor);
